...
export const configSection = "markdown-mermaid-plus";

const defaultMermaidTheme = "default";
const validMermaidThemes = ["base", "forest", "dark", "default", "neutral"];

function sanitizeMermaidTheme(theme: string | undefined) {
  return typeof theme === "string" && validMermaidThemes.includes(theme)
    ? theme
    : defaultMermaidTheme;
}
...
export function injectMermaidConfig(md: MarkdownIt) {
  ...
  md.renderer.render = function (...args) {
    ...
    const configData = {
      ...
      clickDrag: config.get<ClickDragMode>(
        "mouseNavigation.enabled",
        ClickDragMode.Alt,
      ),
      showControls: config.get<ShowControlsMode>(
        "controls.show",
        ShowControlsMode.OnHoverOrFocus,
      ),
