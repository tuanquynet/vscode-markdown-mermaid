...
export class DiagramManager {
  ...
  public setup(id: string, mermaidContainer: HTMLElement): IDisposable {
    ...
    const instance = new DiagramElement(
      id,
      wrapper,
      content,
      this.config,
...
export class DiagramElement {
  ...
  private createZoomControls(): void {
    ...
    controls.innerHTML = `
            ...
            <div class="export-menu-container">
                <button class="export-btn" title="Export" aria-haspopup="true" aria-expanded="false"><span class="codicon codicon-export"></span></button>
    ...
    this.exportButton = controls.querySelector(".export-btn");
    this.exportMenu = controls.querySelector(".export-menu");
    ...
    controls.querySelector(".export-svg-btn")?.addEventListener(
      "click",
      (e) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    controls.querySelector(".export-png-btn")?.addEventListener(
      "click",
      (e) => {
        e.preventDefault();
        e.stopPropagation();
        this.closeExportMenu();
        void this.exportAsPng();
      },
      { signal },
    );
  ...
  private toggleExportMenu(): void {
    if (this.exportMenu?.hasAttribute("hidden")) {
      this.openExportMenu();
    } else {
      this.closeExportMenu();
    }
  ...
  private openExportMenu(): void {
    this.exportMenu?.removeAttribute("hidden");
    this.exportButton?.setAttribute("aria-expanded", "true");
    this.exportButton?.classList.add("active");
  }

  private closeExportMenu(): void {
    this.exportMenu?.setAttribute("hidden", "");
  ...
  public exportAsSvg(): void {
    ...
    try {
      exportDiagramAsSvg(svg, this.id);
    } catch (error) {
  ...
  public async exportAsPng(): Promise<void> {
    ...
    try {
      await exportDiagramAsPng(svg, this.id);
    } catch (error) {
